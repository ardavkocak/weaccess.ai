...
/**
 * Verilen tarih için akışı bulur; yoksa sıranın güncel sahibinden oluşturur.
 * Tamamen senkron (tek işlem).
 *
 * @returns {{ flow: object, created: boolean } | { error: string }}
 */
function ensureFlowForDate(dutyTypeId, source, date) {
...
function ensureFlow(dutyTypeId, source) {
  ...
  return ensureFlowForDate(dutyTypeId, source, calendar.nextWorkingDay());
}

/**
 * Var olan/yeni oluşturulan bir akış için Discord'a soru mesajını gönderir
 * (yeni oluşturulduysa yeni mesaj, zaten varsa mevcut mesaj tazelenir).
 *
 * `startFlow` (cron/"Görev Onayını Başlat") ve `resetTodayAndAskAgain`
 * ("Bugünkü Görevlisini Tekrar Sor") tarafından ortak kullanılır — hedef
 * tarihi BU FONKSİYON belirlemez, çağıran hangi akışı verirse onu gönderir.
 *
 * @param {object} dutyType
 * @param {object} flow
 * @param {boolean} created
 * @param {{ isRetry?: boolean }} [options]
 * @returns {Promise<{ ok: boolean, message: string, skipped?: boolean }>}
 */
async function sendFlowQuestion(dutyType, flow, created, { isRetry = false } = {}) {
  ...
  const content = renderQuestion({
    ...
    template: scheduledMessageService.getDutyMessageFor(dutyType.id)?.message_template ?? '',
...
/**
 * Etkileşimli onay akışını başlatır (veya mevcut olanı tazeler). Her zaman
 * BİR SONRAKİ ÇALIŞMA GÜNÜNÜ hedefler.
 *
 * Cron veya admin "Görev Onayını Başlat" butonu tarafından çağrılır.
 *
 * @param {object} opts
 ...
 * @returns {Promise<{ ok: boolean, message: string, skipped?: boolean }>}
 */
async function startFlow({ dutyTypeId, source = 'cron' }) {
  ...
  return sendFlowQuestion(dutyType, result.flow, result.created);
}
...
/**
 * "Bugünkü Görevlisini Tekrar Sor" (panel butonu).
 *
 * Hedef HER ZAMAN BUGÜNDÜR — `getActiveFlow` (en ileri tarihli akış) veya
 * `calendar.nextWorkingDay()` KULLANILMAZ; otomatik zamanlayıcının/`startFlow`'un
 * tarih mantığıyla hiçbir ilgisi yoktur. Dün "Evet" diyen kişi bugün fiilen
 * ofise gelmemiş olabilir; bu durumda bugünün görevlendirmesi baştan yapılır ve
 * yarının (bir sonraki çalışma gününün) akışına KESİNLİKLE dokunulmaz.
 *
 * "Zaten kesinleşti" kontrolü BİLEREK yapılmaz: bu butonun amacı zaten
 * kesinleşmiş bugünkü görevlendirmeyi yeniden değerlendirmektir. Her basışta
 * yeniden çalışır.
 *
 * İşleyiş:
 *   1. Bugünün akışı var mı? Yoksa sıranın güncel sahibinden sıfırdan açılır.
 *   2. Varsa ve zaten en baştan bekliyorsa (pending, step 0) dokunulmaz —
 *      aşağıda yalnızca Discord mesajı tazelenir.
 *   3. Varsa ve bir sonuca bağlanmışsa (confirmed/exhausted/ilerlemiş pending):
 *      bugünün `duty_history` kaydı silinir, yakılmışsa sıra hakkı iade edilir,
 *      akış sıranın (iade sonrası) güncel sahibinden yeniden başlatılır.
 *
 * DENETİM KAYDI SİLİNMEZ: `duty_confirmation_events` satırları olduğu gibi kalır,
 * yalnızca `reset_count` artar.
 *
 * @returns {Promise<{ ok: boolean, message: string }>}
 */
async function resetTodayAndAskAgain(dutyTypeId) {
  const dutyType = dutyTypeService.getById(dutyTypeId);
  if (!dutyType) return { ok: false, message: 'Görev türü bulunamadı.' };
  ...
  const run = db.transaction(() => {
    const flow = getFlow(dutyTypeId, targetDate);

    if (!flow) {
      return ensureFlowForDate(dutyTypeId, 'manual', targetDate);
    }

    if (flow.status === 'pending' && flow.step === 0) {
      // Bugünün akışı zaten en baştan bekliyor; sıfırlanacak bir cevap yok,
      // aşağıda mevcut mesaj olduğu gibi tazelenir.
      return { flow, created: false };
    }

    // 1) BUGÜNÜN geçmiş kaydını kaldır ve yakılmışsa sırayı iade et.
    const record = historyService.getByDate(dutyTypeId, targetDate);
    if (record) {
      if (record.consumed_turn === 1) {
        rotationService.restoreTurn(dutyTypeId, record.employee_id);
      }
      historyService.removeByDate(dutyTypeId, targetDate);
    }

    // 2) Akışı, sıranın (iade sonrası) güncel sahibinden yeniden başlat.
    const candidate = rotationService.resolveCurrent(dutyTypeId);
    if (!candidate) {
      return { error: 'Aktif çalışan bulunamadı. Önce en az bir çalışanı aktif yapın.' };
    }
    ...
    log.warn(`Bugünün onayı sıfırlandı (akış ${flow.id}): önceki sonuç ${previousName ?? 'yok'}, yeniden ${candidate.full_name} sorulacak.`);

    return { flow: getFlowById(flow.id), created: false, previousName };
  });

  const result = await dbRetry.withRetry(run, { label: 'resetTodayAndAskAgain' });
  if (result.error) return { ok: false, message: result.error };

  const sent = await sendFlowQuestion(dutyType, result.flow, result.created, { isRetry: true });
  if (!sent.ok) {
    return { ok: false, message: `Bugünün onayı sıfırlandı ancak Discord mesajı güncellenemedi: ${sent.message}` };
  }

  const previous = result.previousName ? ` Önceki sonuç (${result.previousName}) iptal edildi.` : '';
  return { ok: true, message: `${sent.message}${previous}` };
}
 ...
 */
async function safeEditReply(interaction, payload, flowId = null, ilog = null) {
  try {
    ...
    ilog?.mark('T5', 'Discord mesajı güncellendi (kurtarma: yeni mesaj gönderildi)');
    return true;
  }
}
 ...
 */
async function safeFollowUp(interaction, payload, ilog = null) {
  try {
    await interaction.followUp(payload);
    ilog?.mark('T5', 'Discord mesajı güncellendi (ephemeral followUp)');
  } catch (error) {
...
async function handleDeferredButton(interaction, parsed, ilog = null) {
  ...
  try {
    ...
    ilog?.mark('T4', 'SQL bitti (tüm denemeler tükendi, hata)');
  ...
  if (outcome.type === 'stale') {
    ...
    await safeFollowUp(interaction, { content: outcome.message, ephemeral: true }, ilog);
    ilog?.step('stale-yaniti-gonderildi');
    return;
  }

  if (outcome.type === 'resolved') {
    await safeEditReply(interaction, { content: outcome.content, components: [] }, parsed.flowId, ilog);
  ...
  await safeEditReply(interaction, { content: outcome.content, components: outcome.components }, parsed.flowId, ilog);
  ilog?.step('discord-guncellendi');
