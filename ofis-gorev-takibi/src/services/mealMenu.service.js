...
/** En son GERÇEKTEN duyurulan menünün tarihi (panelde "Son Gönderim Tarihi" için). */
function getLastAnnouncedDate() {
  const row = db.prepare(`
    SELECT menu_date FROM meal_menus WHERE announced_at IS NOT NULL ORDER BY announced_at DESC LIMIT 1
  `).get();
  return row?.menu_date ?? null;
}
