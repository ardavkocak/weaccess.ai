...
function create(data, position = null) {
  const run = db.transaction(() => {
    ...
    // YENİ ÇALIŞAN, HALEN SÜREN BİR TURU BOZMAMALI — bkz. rotation.service.js
    // dosya başlığı ("tur bazlı" kural). Bir tur zaten devam ediyorsa (o görev
    // türü için `duty_cycle_served`'da satır varsa), yeni kişi bu turda
    // SORULMAMALIDIR; yalnızca bir SONRAKİ turda, herkesle eşit şekilde
    // sisteme dahil olmalıdır.
    //
    // Bunu, yeni çalışanı ilgili tur için "bu turda zaten görev yaptı" olarak
    // ÖNCEDEN işaretleyerek sağlıyoruz — yeni bir sütun/şema değişikliği
    // gerekmez: mevcut `duty_cycle_served` tablosu turun sonunda zaten
    // temizlenir (bkz. rotation.service.completeDuty → allServed → clearServed),
    // bu noktada yeni çalışan da diğerleriyle birlikte doğal olarak "temiz"
    // hale gelip bir sonraki turdan itibaren normal şekilde sorulmaya başlar.
    // Round henüz hiç başlamadıysa (served boşsa) özel bir işlem GEREKMEZ —
    // yeni çalışan zaten sırasıyla normal şekilde ilk turdan itibaren sorulur.
    if (created.is_active === 1) {
      const dutyTypeService = require('./dutyType.service'); // lazy: döngüsel bağımlılığı önler
      const rotationService = require('./rotation.service'); // lazy: döngüsel bağımlılığı önler
      for (const dutyType of dutyTypeService.getAll()) {
        if (rotationService.getServedIds(dutyType.id).size > 0) {
          rotationService.markServed(dutyType.id, created.id);
        }
      }
    }
