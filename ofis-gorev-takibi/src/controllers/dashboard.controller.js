 ...
 *
 * "Bugünkü Görevlisini Tekrar Sor": hedef HER ZAMAN bugündür, yarının (bir
 * sonraki çalışma gününün) akışına dokunmaz. Günün geçmiş kaydı silinir,
 * yakılmışsa sıra hakkı iade edilir ve Discord'daki soru baştan sorulur.
 * Denetim kayıtları korunur. Ayrıntı: dutyConfirmation.service.resetTodayAndAskAgain().
 */
const resetConfirmation = asyncHandler(async (req, res) => {
  ...
  const result = await confirmationService.resetTodayAndAskAgain(dutyTypeId);
  req.flash(result.ok ? 'success' : 'error', result.message);
