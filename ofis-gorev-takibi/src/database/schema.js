...
function createTables() {
  db.exec(`
    ...
    -- company: Yemek Sistemi'nin katılım listelerini şirkete göre gruplaması
    -- için elle girilen serbest metin alanı (bkz. office-portal Employee formu).
...
/**
 * Göç: `employees.company`.
 *
 * Yemek Sistemi'nin katılım listelerini şirkete göre gruplayabilmesi için
 * office-portal'daki çalışan formuna eklenen elle-girilen alan. Mevcut
 * kayıtlarda NULL kalır — office-portal Görev Takip ekranından tek tek
 * doldurulur.
 */
function migrateEmployeeCompany() {
  if (hasColumn('employees', 'company')) return;

  log.info('Göç uygulanıyor: employees.company ekleniyor');
  db.exec('ALTER TABLE employees ADD COLUMN company TEXT');
  log.info('Göç tamamlandı.');
}
...
function seedMealSettings() {
  db.prepare("INSERT INTO settings (key, value) VALUES ('meal_notify_time', ?) ON CONFLICT(key) DO NOTHING")
    .run(config.seedSettings.meal_notify_time || '10:00');
  db.prepare("INSERT INTO settings (key, value) VALUES ('meal_notify_enabled', '1') ON CONFLICT(key) DO NOTHING").run();
}
